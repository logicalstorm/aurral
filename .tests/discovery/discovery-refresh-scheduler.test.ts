...
const honkerDbModule = await importFromRepo("backend/services/honkerDb.ts");
const {
...
} = await importFromRepo("backend/services/discoveryRefreshScheduler.ts");
const { getDiscoveryCache } = await importFromRepo("backend/services/discoveryService.ts");
...
test("pruneDuplicateScheduledDiscoveryRefreshes collapses stacked future refreshes", async () => {
  ...
  const { pruneDuplicateScheduledDiscoveryRefreshes } = await importFromRepo(
    "backend/services/discoveryRefreshScheduler.ts",
  );
