...
- Confirm at least one download client is configured in **Settings → Downloads** (slskd and/or Prowlarr + NZBGet).
...
- Set **Settings → Downloads → Downloads folder** to a path under that mount, for example `/music/Aurral`, or set `DOWNLOAD_FOLDER=/music/Aurral` in compose.
...
- If needed, set `PATH_MAPPINGS=N:/ServerFolders/Music|/music` in compose, or add mappings under **Settings → Downloads → Advanced remote path mappings**. Compose `PATH_MAPPINGS` work at runtime but are not shown in the UI.
...
- For NZBGet on Windows, set **Completed download path** in **Settings → Downloads → NZBGet** if Aurral cannot detect the folder automatically.
