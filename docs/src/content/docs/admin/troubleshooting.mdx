...
- If Navidrome runs outside Docker, enable **Settings → Playback → Navidrome → Use host paths in playlist files** or set `M3U_PATH_MODE=remote`.
...
- Enable **Settings → Playback → Navidrome → Use host paths in playlist files**, or set `M3U_PATH_MODE=remote` in compose and recreate the Aurral container.
