...
- Confirm at least one download client is configured in **Settings → Download Clients** (slskd and/or Prowlarr + NZBGet).
- For slskd: check connection and Soulseek login status.
- For Usenet: confirm Prowlarr indexers are enabled in Aurral and NZBGet can reach completed files.
- Open the **slskd** card and try disabling **Strict format only**, or switch preferred format to MP3 if FLAC matches are scarce.
- Check job states on the Playlists page and **History**.

## Files do not appear in Navidrome

- Confirm the shared root containing `DOWNLOAD_FOLDER` is mounted into Aurral, Navidrome, slskd, and NZBGet.
...
Symptoms include logs such as `Lidarr track exists but file is not accessible from Aurral`, completed slskd or NZBGet files not moving, or **Test library access** failing while Lidarr itself works.
...
- Set **Settings → Download Clients → Downloads Folder** to a path under that mount, for example `/music/Aurral`, or set `DOWNLOAD_FOLDER=/music/Aurral` in compose.
...
- Run **Settings → Library → Lidarr → Test library access** to see the exact path Aurral cannot read.
- If needed, set `PATH_MAPPINGS=N:/ServerFolders/Music|/music` in compose, or add mappings under **Settings → Download Clients → Remote Path Mappings**. Compose `PATH_MAPPINGS` work at runtime but are not shown in the UI.
- If Navidrome sees the files at a different path than Aurral, enable **Settings → Playback → Navidrome → Use Navidrome paths in M3U files** and add a Navidrome path mapping, or set `M3U_PATH_MODE=remote` plus `M3U_PATH_MAPPINGS`.
...
- Enable **Settings → Playback → Navidrome → Use Navidrome paths in M3U files** and add a Navidrome path mapping, or set `M3U_PATH_MODE=remote` plus `M3U_PATH_MAPPINGS` in compose and recreate the Aurral container.
- Refresh playlists so `.m3u` files are rewritten by re-running or saving a flow, or by editing and saving a shared playlist.
...
Leave **Use Navidrome paths in M3U files** off when Navidrome is in Docker with the same mounts as Aurral.
...
- If Plex and Aurral use different mounts, set **Settings → Playback → Plex → Plex downloads path** to the folder Plex sees (without `/aurral-weekly-flow`). Leave it blank when both services share the same path prefix.
...
- **Use Navidrome paths in M3U files** under Navidrome does not apply to Plex.
