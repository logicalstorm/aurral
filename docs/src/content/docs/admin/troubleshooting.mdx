...
- Run **Settings → Library → Lidarr → Test library access** so Aurral can auto-detect path mappings.
- If needed, set `PATH_MAPPINGS=N:/ServerFolders/Music|/music` in compose, or add mappings under **Settings → Downloads → Path mappings**. Compose `PATH_MAPPINGS` work at runtime but are not shown in the UI.
- If Navidrome sees the files at a different path than Aurral, enable **Settings → Playback → Navidrome → Use Navidrome-visible paths in playlist files** or set `M3U_PATH_MODE=remote`.
...
- Enable **Settings → Playback → Navidrome → Use Navidrome-visible paths in playlist files**, or set `M3U_PATH_MODE=remote` in compose and recreate the Aurral container.
...
Leave **Use Navidrome-visible paths in playlist files** off when Navidrome is in Docker with the same mounts as Aurral.
...
- **Use Navidrome-visible paths in playlist files** under Navidrome does not apply to Plex.
