...
- Set **Settings → Playlists → Path** to a folder under that mount, for example `/music/Aurral`.
- Run **Settings → Integrations → Lidarr → Test library access** so Aurral can auto-detect path mappings.
- If needed, add mappings manually under **Settings → Playlists → Path mappings**.
