...
If Lidarr runs on Windows and Aurral runs in Docker, run **Test library access** in **Settings → Library → Lidarr** to see the exact reported path, then add a manual Lidarr mapping under **Settings → Download Clients → Remote Path Mappings** if the path differs inside Aurral. See [Shared storage](/getting-started/storage/#windows-and-mixed-docker-setups).
