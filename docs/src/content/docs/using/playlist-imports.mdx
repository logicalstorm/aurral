...
If Lidarr runs on Windows and Aurral runs in Docker, use path mappings under **Settings → Downloads** or run **Test library access** in **Settings → Library → Lidarr** to auto-detect them. See [Shared storage](/getting-started/storage/#windows-and-mixed-docker-setups).

When reuse points at Lidarr files and Navidrome sees those files at a different path than Aurral, also enable **Use Navidrome-visible paths in playlist files** under **Settings → Playback → Navidrome** so generated M3U files reference paths Navidrome can open. See [Navidrome](/integrations/navidrome/#mixed-windows-and-docker-navidrome).
