...
If Lidarr runs on Windows and Aurral runs in Docker, use path mappings under **Settings → Playlists** or run **Test library access** in Lidarr settings to auto-detect them. See [Shared storage](/getting-started/storage/#windows-and-mixed-docker-setups).

When reuse points at Lidarr files and Navidrome runs on Windows (not in Docker), also enable **Use host paths in playlist files** under **Settings → Integrations → Navidrome** so generated M3U files reference host paths. See [Navidrome](/integrations/navidrome/#mixed-windows-and-docker-navidrome).
