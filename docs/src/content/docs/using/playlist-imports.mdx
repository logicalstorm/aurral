...
When reuse points at Lidarr files and Navidrome runs on Windows (not in Docker), also enable **Use host paths in playlist files** under **Settings → Playback → Navidrome** so generated M3U files reference host paths. See [Navidrome](/integrations/navidrome/#mixed-windows-and-docker-navidrome).
