...
If Lidarr runs on Windows and Aurral runs in Docker, run **Test library access** in **Settings → Library → Lidarr** to see the exact reported path, then add a mapping under **Settings → Downloads → Advanced remote path mappings**. See [Shared storage](/getting-started/storage/#windows-and-mixed-docker-setups).
