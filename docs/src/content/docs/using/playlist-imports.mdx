...
If Lidarr runs on Windows and Aurral runs in Docker, run **Test library access** in **Settings → Library → Lidarr** to see the exact reported path, then add a manual Lidarr mapping under **Settings → Downloads → Advanced remote path mappings** if the path differs inside Aurral. See [Shared storage](/getting-started/storage/#windows-and-mixed-docker-setups).

When reuse points at Lidarr files and Navidrome sees those files at a different path than Aurral, also enable **Use Navidrome paths in M3U files** under **Settings → Playback → Navidrome** and add a Navidrome path mapping so generated M3U files reference paths Navidrome can open. See [Navidrome](/integrations/navidrome/#mixed-windows-and-docker-navidrome).
