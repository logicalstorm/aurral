...
Deep dive changes how far Aurral looks when pulling candidates for a source. **Off** keeps source pulls narrower. **On** lets Aurral reach further from each source. It does not change the schedule or source percentages.
