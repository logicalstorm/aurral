...
Path mappings apply to Aurral reading files. If Navidrome runs outside Docker, enable **Use host paths in playlist files** under **Settings → Playback → Navidrome** so M3U playlists use host paths instead. See [Navidrome](/integrations/navidrome/#mixed-windows-and-docker-navidrome).
