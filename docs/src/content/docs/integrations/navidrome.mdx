...
| --- | --- | --- |
| Docker, same mounts as Aurral (for example `/data` or `/music`) | **local** (default) | Leave **Use Navidrome-visible paths in playlist files** off |
| Docker, different mount path than Aurral (for example Navidrome sees `/data/media/music` while Aurral sees `/music`) | **remote** | Enable the Navidrome setting or set `M3U_PATH_MODE=remote` |
...
**Use Navidrome-visible paths in playlist files** (or `M3U_PATH_MODE=remote`) reverses paths **only when writing M3U files**:
