...
| Path mismatch fix | **Use Navidrome-visible paths in playlist files** / `M3U_PATH_MODE` | **Plex downloads path** (optional) |
| Playlist names | `[A] Flow name`, `[AS] Shared name` | Bare flow or playlist name |

Navidrome's **Use Navidrome-visible paths in playlist files** setting does not affect Plex. It only changes paths written inside `.m3u` files.
