...
In Aurral onboarding, you can optionally apply **Davo's Community Lidarr Guide** — an Aurral-friendly quality profile, custom formats, and naming scheme. You can also apply it later from **Settings → Library → Lidarr**.
...
For **Aurral flows and playlists**, connect download clients in **Settings → Downloads**:
...
- **Prowlarr + NZBGet** for Usenet — Aurral searches indexers via Prowlarr, queues NZBs in NZBGet, and imports completed files the same way. See [Usenet](/integrations/usenet/).
...
6. Move standout tracks into a [static Aurral playlist](/using/playlist-imports/).
...
On the **Playlists** page, choose how generated playlists handle files that already exist:
