...
function buildIpv4Subnet(address: string, netmask: string, interfaceName: string) {
...
function getRequestIps(req: Request) {
  const ips = [
...
function toResolvedUser(user: Record<string, unknown>) {
  if (!user) return null;
  return {
    id: user.id,
...
const toSessionUser = (session: Record<string, unknown>) => {
...
export const resolveSessionUserFromToken = (token: string | null) => {
  if (!token) return null;
  return toSessionUser(getSessionByToken(token) as unknown as Record<string, unknown>);
};
...
export function isRequestFromTrustedLocalSubnet(req: any) {
  ...
  const resolvedSubnet = subnet;
  return requestIps.some((ip) => {
    if (!isPrivateIpv4(ip)) return false;
    const ipInt = ipv4ToInt(ip);
    if (ipInt == null) return false;
    return (ipInt & ipv4ToInt(resolvedSubnet.netmask)!) >>> 0 === resolvedSubnet.networkInt;
  });
}
...
export function resolveProxyUser(req: Request) {
...
function resolveUser(username: string, password: string) {
  const users = userOps.getAllUsers();
  if (users.length === 0) {
    migrateLegacyAdmin();
  }
...
function legacyAuth(username: string, password: string) {
...
export function resolveLocalNetworkBypassUser(req: Request) {
  const status = getLocalNetworkBypassStatus(req);
  if (!status.active) return null;
  return toResolvedUser(getSoleAdminUser() as unknown as Record<string, unknown>);
}
...
function consumeStreamToken(rawToken: string) {
...
export const verifyTokenAuth = (req: Request) => {
  const user = resolveRequestUser(req);
  if (user) {
    req.user = user;
    return true;
