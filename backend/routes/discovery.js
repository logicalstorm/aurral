...
const fetchLastfmTopTagNames = async () => {
  ...
  return tags.map((tag) => (tag.name != null ? String(tag.name).trim() : "")).filter(Boolean);
};
...
const _normalizeTextList = (value) => {
...
router.get("/", requireAuth, async (req, res) => {
  ...
  globalTop = globalTop.filter((artist) => !isLibraryArtist(artist, existingArtistKeys));
  ...
  const playlists = annotateDiscoverPlaylistsForUser(discoverPlaylists, req.user).filter(
    (playlist) => playlist.trackCount > 0,
  );

  const playlistBuildStatus = getDiscoveryPlaylistBuildStatus(effectiveCacheNamespace);
...
router.get("/tags", async (req, res) => {
  try {
    ...
    if (prefix.length >= 2 && !filtered.some((name) => name.toLowerCase() === prefix)) {
...
router.get("/nearby-shows", requireAuth, async (req, res) => {
  try {
    ...
    const configuredRadius = Number(settings.integrations?.ticketmaster?.searchRadiusMiles);
  ...
  try {
    ...
    res.json({
      ...
      preferences: {
        ...
        localDiscoveryIncludeRecommendations:
          nextSettings.integrations?.ticketmaster?.localDiscoveryIncludeRecommendations !== false,
        localDiscoveryIncludeTrending:
          nextSettings.integrations?.ticketmaster?.localDiscoveryIncludeTrending !== false,
      },
    });
  } catch (error) {
    res.status(500).json({
...
router.delete("/feedback/:id", requireAuth, (req, res) => {
  const feedbackList = removeDiscoveryFeedback(req.user?.id || "global", req.params.id);
  res.json({
    success: true,
    feedbackList,
  });
