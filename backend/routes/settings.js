...
router.post("/", async (req, res) => {
  try {
    ...
    const localBypassWasEnabled = currentSettings?.security?.localNetworkBypass?.enabled === true;
    ...
    if (integrations?.metadata) {
      ...
      nextMetadata.userAgentSuffix =
        typeof nextMetadata.userAgentSuffix === "string" ? nextMetadata.userAgentSuffix.trim() : "";
    ...
    if (integrations?.nzbget) {
      ...
      nextNzbget.password = typeof nextNzbget.password === "string" ? nextNzbget.password : "";
      nextNzbget.category = String(nextNzbget.category || "aurral").trim() || "aurral";
      const priority = Number.parseInt(nextNzbget.priority, 10);
      nextNzbget.priority = Number.isFinite(priority) ? Math.min(1000, Math.max(1, priority)) : 20;
    ...
    if (integrations?.navidrome) {
      integrations.navidrome.m3uPathMode = normalizeM3uPathMode(integrations.navidrome.m3uPathMode);
    ...
    if (integrations) {
      mergedIntegrations = {
        ...
        gotify: integrations.gotify
          ...
          : mergedIntegrations.gotify,
        webhooks:
          integrations.webhooks !== undefined ? integrations.webhooks : mergedIntegrations.webhooks,
    ...
    if (localBypassWasEnabled && reconciled?.security?.localNetworkBypass?.enabled !== true) {
...
router.post("/gotify/test", async (req, res) => {
  try {
    ...
    const msg = error.response?.data?.description || error.response?.data?.error || error.message;
    res
...
router.get("/browse", async (req, res) => {
  try {
    ...
    res.status(400).json({ error: "Cannot read path", message: error.message });
  }
});
