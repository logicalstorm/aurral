...
export default function registerArtists(router: any) {
  router.get('/artists', cacheMiddleware(120), async (req: any, res: any) => {
    try {
  ...
  router.get('/artists/:mbid', cacheMiddleware(120), async (req: any, res: any) => {
    try {
  ...
  router.post('/artists', requireAuth, requirePermission('addArtist'), async (req: any, res: any) => {
    try {
      const {
      ...
      try {
        ...
        if ((artist as any)?.error) {
          return res.status(503).json({
            error: (artist as any).error,
            message: (artist as any).error,
          });
        }
        const { lidarrClient } = await import('../../../services/lidarrClient.js');
        if (lidarrClient && lidarrClient.isConfigured()) {
          if ((artist as any).monitorOption && (artist as any).monitorOption !== 'none') {
    ...
    async (req: any, res: any) => {
      try {
        ...
        if (lidarrClient && lidarrClient.isConfigured()) {
          ...
          if (lidarrArtist && (lidarrArtist as any).monitor !== 'none' && (lidarrArtist as any).monitored) {
