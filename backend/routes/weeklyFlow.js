...
const normalizeFlowStringArray = (value) => {
  if (Array.isArray(value)) {
    return [...new Set(value.map((entry) => getFlowEntryName(entry)).filter(Boolean))];
  }
...
const getAccessibleFlow = (user, flowId) => flowPlaylistConfig.getFlowForUser(user, flowId);
...
const filterJobsForUser = (user, jobs) =>
  (Array.isArray(jobs) ? jobs : []).filter((job) => canAccessPlaylistType(user, job?.playlistType));
