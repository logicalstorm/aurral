...
function canBrowseFilesystem(req: unknown) {
...
router.get('/browse', noCache, (req, res) => {
  try {
    ...
    return res.status(400).json({
      error: 'browse_failed',
      message: (error as Error)?.message || 'Failed to browse storage path.',
    });
  }
});
...
router.post('/ensure', noCache, (req, res) => {
  try {
    ...
    return res.json({
      ...result,
      path: ensured.path,
      created: !!(ensured as Record<string, unknown>).created,
    });
  } catch (error) {
    return res.status(400).json({
      error: 'ensure_failed',
      message: (error as Error)?.message || 'Failed to prepare folder path.',
    });
  }
});
