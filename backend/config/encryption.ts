...
function setAt(obj: Record<string, any>, path: readonly string[], value: unknown) {
...
export function decryptIntegrations(integrations: Record<string, unknown> | null | undefined, key: string) {
...
export function encryptIntegrations(integrations: Record<string, unknown> | null | undefined, key: string) {
  if (!integrations || typeof integrations !== "object") return integrations;
  const out = JSON.parse(JSON.stringify(integrations)) as Record<string, unknown>;
  for (const path of SENSITIVE_PATHS) {
    const v = getAt(out, path);
