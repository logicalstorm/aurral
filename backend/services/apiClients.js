...
const _itunesAlbumArtCache = new NodeCache({
  stdTTL: 24 * 60 * 60,
  checkperiod: 10 * 60,
  maxKeys: 2000,
});
...
export const getMusicBrainzContact = () => {
  const settings = dbOps.getSettings();
  return (
    settings.integrations?.musicbrainz?.email || process.env.CONTACT_EMAIL || "user@example.com"
  );
};
...
const runMetadataProviderHealthProbes = () => Promise.allSettled([probeMusicbrainzHostedHealth()]);
...
const _ensureMetadataProviderProbeLoop = () => {
  if (metadataProviderProbeTimer) return;

  const tick = async () => {
    await runMetadataProviderHealthProbes();
    clearInterval(metadataProviderProbeTimer);
    metadataProviderProbeTimer = setInterval(tick, getMetadataProviderProbeIntervalMs());
...
const requestMusicbrainz = async (baseUrl, endpoint, queryParams, userAgent, forceIpv4 = false) =>
...
const _configureMusicbrainzLimiter = async () => {
...
const _itunesLimiter = new Bottleneck({
  reservoir: 20,
...
const _musicbrainzInflightRequests = new Map();
...
const musicbrainzRequestWithRetry = async (
  ...
  let error;
  try {
    const response = await requestMusicbrainz(baseUrl, endpoint, queryParams, userAgent, forceIpv4);
...
const _normalizeItunesArtworkUrl = (url) =>
  String(url || "")
    .trim()
...
export const lastfmRequest = lastfmLimiter.wrap(async (method, params = {}, options = {}) => {
...
export const listenbrainzRequest = listenbrainzLimiter.wrap(async (path, params = {}) => {
...
function getTrackMedium(track) {
  const numeric = Number.parseInt(
    track?.mediumNumber ?? track?.mediumnumber ?? track?.disk_number ?? track?.diskNumber,
    10,
  );
  return Number.isFinite(numeric) && numeric > 0 ? numeric : null;
}
  ...
  const ranked = deezerAlbums
    .map((album) => {
      ...
      if (albumTitle === targetTitle) {
        score += 100;
      } else if (albumTitle.includes(targetTitle) || targetTitle.includes(albumTitle)) {
        score += 45;
      }
...
const _ALLOWED_PRIMARY_TYPES = new Set(["album", "ep", "single"]);

function _normalizeArtistReleaseTypeSelection(selectedReleaseTypes = []) {
  ...
  const primaryTypes = PRIMARY_RELEASE_TYPES.filter((type) => normalized.has(type));
  const secondaryTypes = SECONDARY_RELEASE_TYPES.filter((type) => normalized.has(type));
  return {
    primaryTypes,
    secondaryTypes,
  };
...
function _buildArtistReleaseGroupQuery(mbid, primaryTypes, secondaryTypes) {
  ...
  if (Array.isArray(primaryTypes) && primaryTypes.length > 0) {
    if (primaryTypes.length === 1) {
      ...
      clauses.push(
        `(${primaryTypes.map((type) => `primarytype:${type.toLowerCase()}`).join(" OR ")})`,
      );
    }
  }
...
export async function musicbrainzGetArtistReleaseGroups(
  ...
  try {
    ...
    const mapped = items.map((item) => ({
      ...
      "secondary-types": Array.isArray(item.secondaryTypes) ? item.secondaryTypes : [],
...
const officialMusicbrainzRecordingSearch = async (mbid, { limit = 100, offset = 0 } = {}) => {
  const contact = (getMusicBrainzContact() || "").trim() || "https://github.com/aurral";
  const userAgent = `${APP_NAME}/${APP_VERSION} ( ${contact} )`;
...
export async function musicbrainzGetArtistAppearsOnReleaseGroups(
  ...
  if (!mbid) return [];
  const safeLimit = Math.min(250, Math.max(1, Number.parseInt(limit, 10) || 24));
...
export async function resolveDeezerAlbumToMbid(artistName, albumName, deezerAlbumId) {
