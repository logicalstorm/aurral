...
export function normalizeListenHistoryProvider(value: any) {
...
export function normalizeListenHistoryUsername(value: any) {
  if (value == null) return null;
  const normalized = String(value).trim();
  return normalized || null;
}

export function normalizeListenHistoryUrl(value: any) {
  if (value == null) return null;
  const normalized = String(value).trim().replace(/\/+$/, '');
  return normalized || null;
}

export function getListenHistoryProfile(source: any = {}) {
  const safeSource: Record<string, any> = source && typeof source === 'object' && !Array.isArray(source) ? source : {};
...
export function hasListenHistoryProfile(profile: any) {
...
export function listenHistoryProfilesEqual(a: any, b: any) {
...
export function getListenHistoryCacheNamespace(profile: any) {
  const normalized = getListenHistoryProfile(profile);
  if (normalized.listenHistoryProvider === 'koito') {
  ...
  const prefix = CACHE_PREFIX_BY_PROVIDER[normalized.listenHistoryProvider as keyof typeof CACHE_PREFIX_BY_PROVIDER];
  return prefix ? `${prefix}:${normalized.listenHistoryUsername}` : null;
}

export function getDefaultListenHistoryProfile(settings: any) {
...
export function resolveListenHistorySettings(user: any = {}, settings: any = null) {
