...
const SHARE_REFRESH_DEBOUNCE_MS = 1000;
const AUDIO_SHARE_EXTENSIONS = new Set([
  ".mp3",
  ".flac",
  ".m4a",
  ".aac",
...
const resolveWeeklyFlowRoot = () => {
  const root = process.env.WEEKLY_FLOW_FOLDER || "/app/downloads";
...
export class SimpleSoulseekClient {
  constructor() {
    ...
    this.connectedShareConfigKey = null;
    this.needsReconnect = false;
    this.shareRefreshDirty = false;
  ...
  _serializeCredentialConfig(config = this.config) {
  ...
  _serializeShareConfig(config = this.config) {
    const sharedFolders = Array.isArray(config?.sharedFolders)
      ? [...config.sharedFolders].map((entry) => String(entry || "")).sort()
      : [];
    return JSON.stringify({
      shareDownloads: config?.shareDownloads === true,
      sharedFolders,
    });
  }

  _getShareBaseRoot() {
    return path.resolve(resolveWeeklyFlowRoot(), "aurral-weekly-flow");
  }

  isSharingEnabled() {
    const settings = dbOps.getSettings();
    return settings?.weeklyFlowWorker?.shareDownloads === true;
  }
  ...
  getShareRoots() {
    ...
    const baseRoot = this._getShareBaseRoot();
    let entries = [];
    try {
      entries = fsNode.readdirSync(baseRoot, { withFileTypes: true });
    } catch {
      return [];
    }
    return entries
      ...
      .map((name) => path.resolve(baseRoot, name))
      .filter((dirPath) => {
        try {
          return fsNode.statSync(dirPath).isDirectory();
        } catch {
          return false;
        }
      })
  ...
  _countFilesInRoots(sharedFolders = []) {
    let total = 0;
    const scan = (dirPath) => {
      let entries = [];
      try {
        entries = fsNode.readdirSync(dirPath, { withFileTypes: true });
      } catch {
        return;
      }
      for (const entry of entries) {
        ...
        if (entry.isFile() && AUDIO_SHARE_EXTENSIONS.has(path.extname(name).toLowerCase())) {
          total += 1;
        }
      }
    };
    for (const folder of Array.isArray(sharedFolders) ? sharedFolders : []) {
      scan(folder);
    }
    return total;
  }
  ...
  updateConfig() {
    const previousCredentialKey = this._serializeCredentialConfig(this.config);
    const previousShareKey = this._serializeShareConfig(this.config);
    const { username, password, autoGenerated } = this.ensureCredentials();
    const shareDownloads = this.isSharingEnabled();
    const sharedFolders = shareDownloads ? this.getShareRoots() : [];
    this.config = {
      username,
      password,
      autoGenerated,
    ...
    const nextCredentialKey = this._serializeCredentialConfig(this.config);
    const nextShareKey = this._serializeShareConfig(this.config);
    this.needsReconnect =
      this.connected === true &&
      this.client !== null &&
      ((this.connectedConfigKey !== null &&
        this.connectedConfigKey !== nextCredentialKey) ||
        (this.connectedShareConfigKey !== null &&
          this.connectedShareConfigKey !== nextShareKey));
    return (
      previousCredentialKey !== nextCredentialKey ||
      previousShareKey !== nextShareKey
    );
  }
  ...
  refreshShares({ reconnectIfNeeded = true } = {}) {
    const changed = this.updateConfig();
    if (!changed) {
  ...
  clearShares({ reconnectIfNeeded = true } = {}) {
    const previousShareKey = this._serializeShareConfig(this.config);
    const nextConfig = {
      ...(this.config || {}),
    ...
    const nextShareKey = this._serializeShareConfig(this.config);
    const changed = previousShareKey !== nextShareKey;
    this.needsReconnect =
      changed &&
      this.connected === true &&
      this.client !== null &&
  ...
  async _flushShareChanges() {
    ...
    do {
      this.shareRefreshDirty = false;
      result = await this.refreshShares({ reconnectIfNeeded: true });
    } while (this.shareRefreshDirty);
    return result;
  }

  applyShareChanges() {
    this.shareRefreshDirty = true;
    if (this.shareRefreshPromise) {
      return this.shareRefreshPromise;
    }
    this.shareRefreshPromise = new Promise((resolve, reject) => {
      setTimeout(async () => {
        try {
          const result = await this._flushShareChanges();
          resolve(result);
        } catch (error) {
          reject(error);
        } finally {
  ...
  async connect(retryInvalidPass = true) {
    this.updateConfig();
    const credentialConfigKey = this._serializeCredentialConfig(this.config);
    const shareConfigKey = this._serializeShareConfig(this.config);
    if (
      this.connected &&
      this.client &&
      !this.needsReconnect &&
      this.connectedConfigKey === credentialConfigKey &&
      this.connectedShareConfigKey === shareConfigKey
    ) {
      return true;
    }
      ...
      slsk.connect(
        {
          ...
          sharedFolders: this.config.sharedFolders || [],
        },
        async (err, client) => {
          if (err) {
            this.connected = false;
  ...
  getStatus() {
    const summary =
      this.client && typeof this.client.getSharedSummary === "function"
        ? this.client.getSharedSummary()
        : null;
    const sharedFolders = Array.isArray(this.config?.sharedFolders)
      ? this.config.sharedFolders
      : [];
    return {
      connected: this.isConnected(),
      connectionRefCount: this.connectionRefCount,
      keepAliveArmed: this.idleDisconnectTimer !== null,
      sharingEnabled: this.config?.shareDownloads === true,
      sharedFolderCount:
        Number(summary?.folderCount) ||
        sharedFolders.length,
      sharedFileCount:
        Number(summary?.fileCount) ||
        this._countFilesInRoots(sharedFolders),
      metrics: {
