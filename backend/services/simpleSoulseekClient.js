...
export class SimpleSoulseekClient {
  ...
  _serializeConfig(config = this.config) {
    return JSON.stringify({
      ...
      seedDownloads: config?.seedDownloads === true,
      sharedFolders: Array.isArray(config?.sharedFolders)
        ? config.sharedFolders.map((folder) => String(folder || ""))
        : [],
    });
  }

  _shouldPersistConnection(config = this.config) {
    return config?.seedDownloads === true;
  }
  ...
  getConnectionSettings() {
    const settings = dbOps.getSettings();
    const seedDownloads = settings?.weeklyFlowWorker?.seedDownloads !== false;
    return {
      seedDownloads,
      sharedFolders: seedDownloads ? [resolveSharedDownloadsFolder()] : [],
    };
  }
  ...
  updateConfig() {
    ...
    const { seedDownloads, sharedFolders } = this.getConnectionSettings();
    this.config = {
      username,
      password,
      autoGenerated,
  ...
  async applyConfigChanges() {
    ...
    if (this._shouldPersistConnection()) {
      await this.connect();
    }
  ...
  async connect(retryInvalidPass = true) {
    ...
    for (const sharedFolder of this.config.sharedFolders || []) {
