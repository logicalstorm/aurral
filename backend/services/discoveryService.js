...
export const getDiscoveryAutoRefreshHours = () => {
  const settings = dbOps.getSettings();
  const parsed = parseInt(settings.integrations?.lastfm?.discoveryAutoRefreshHours, 10);
...
export const getDiscoveryRecommendationsPerRefresh = () => {
  const settings = dbOps.getSettings();
  const parsed = parseInt(settings.integrations?.lastfm?.discoveryRecommendationsPerRefresh, 10);
...
const getDiscoveryCandidateLimit = () =>
  Math.min(
    ...
    Math.max(
      160,
      Math.ceil(getDiscoveryRecommendationsPerRefresh() * DISCOVERY_CANDIDATE_MULTIPLIER),
    ),
  );
...
const getDiscoveryTagSeedLimit = (count, failureRatio) => {
  ...
  const sampleBase = Math.min(count, Math.max(25, Math.min(45, Math.ceil(target / 5))));
...
const getCandidateTagHydrationLimit = (count, failureRatio, depth = 1) => {
  ...
  if (failureRatio >= 0.3) {
    return Math.min(count, depth >= 2 ? 24 : Math.max(80, Math.ceil(target * 0.75)));
  }
  return Math.min(
    count,
    depth >= 2 ? getSecondHopRecommendationLimit() : Math.max(target, Math.ceil(target * 1.5)),
  );
};

export const getDiscoveryFlowsPerRefresh = () => {
  const settings = dbOps.getSettings();
  const parsed = parseInt(settings.integrations?.lastfm?.discoveryFlowsPerRefresh, 10);
...
const buildTasteProfile = ({
  ...
  const bucketedHistorySeeds = historyArtists.map((artist, index) => ({
    ...
    affinityWeight:
      1.35 + Math.min(1.2, Math.log10(Math.max(0, Number(artist.playcount || 0)) + 1) * 0.35),
  }));
...
export const recordDiscoveryUpdateProgress = (phase, progressMessage, progress, extra = {}) => {
...
export const getDiscoveryPlaylistBuildStatus = (cacheNamespace = null) => {
  ...
  const message = cacheFlag
    ? discoveryCache.playlistsUpdateMessage || "Updating recommended playlists..."
...
const emitDiscoveryProgress = (phase, progressMessage, progress, extra = {}) => {
  ...
  if (profile.listenHistoryProvider === "listenbrainz") {
    ...
    return artists
      .map((artist) => {
        ...
        const resolvedMbid = mbid || musicbrainzGetCachedArtistMbidByName(artist.artist_name);
        return {
...
const collectSeedTagsAndGenres = async (seeds, lastfmHealth, progressPhase = null) => {
  ...
  let tagsFound = 0;
  await mapWithConcurrency(seeds, DISCOVERY_NETWORK_CONCURRENCY, async (seed) => {
    try {
      ...
      console.warn(`Failed to get Last.fm tags for ${seed.artistName}: ${error.message}`);
    }
  });
...
const fetchArtistTagNames = async (artist, lastfmHealth) => {
  ...
  const data = await lastfmRequest("artist.getTopTags", mbid ? { mbid } : { artist: artistName });
  recordLastfmResult(lastfmHealth, data);
  if (!data?.toptags?.tag) return [];
...
const resolveRecommendationCandidates = async (
  ...
  const shortlist = recommendations.slice(0, Math.min(recommendations.length, resolveLimit));

  await mapWithConcurrency(shortlist, DISCOVERY_NETWORK_CONCURRENCY, async (item) => {
  ...
  return merged.slice(0, Math.max(120, getDiscoveryRecommendationsPerRefresh() * 2));
};
...
const buildRecommendationsFromSeeds = async ({
  ...
  await mapWithConcurrency(seeds, DISCOVERY_NETWORK_CONCURRENCY, async (seed) => {
    try {
      ...
      if (sourceTags.length > 0) {
        ...
        if (tagData?.toptags?.tag) {
          const tags = Array.isArray(tagData.toptags.tag)
            ? tagData.toptags.tag
            : [tagData.toptags.tag];
          sourceTags = normalizeSeedTagList(tags.map((tag) => tag?.name));
        }
      }
      ...
      console.warn(`Error getting similar artists for ${seed.artistName}: ${error.message}`);
    }
  });
  ...
  await mapWithConcurrency(bridgeSeeds, DISCOVERY_NETWORK_CONCURRENCY, async (bridge) => {
    try {
      ...
      console.warn(`Error getting second-hop similar artists for ${bridge.name}: ${error.message}`);
    }
  });
  ...
  secondHopList = rerankRecommendations(secondHopList, getSecondHopRecommendationLimit(), {
    discoveryMode,
  });
...
const getDiscoveryPlaylistBuildKey = (cacheNamespace = null) => String(cacheNamespace || "global");

const buildDiscoveryUpdatePayload = (
  discoveryData,
  { phase = "completed", progress = 100, progressMessage = "Discovery refresh completed" } = {},
) => {
  ...
  return {
    ...
    recommendationQuality:
      discoveryData.recommendationQuality || discoveryData.metadata?.recommendationQuality || null,
    isEnriching: discoveryData.isEnriching === true || discoveryData.metadata?.isEnriching === true,
    discoveryRunId: discoveryData.discoveryRunId || discoveryData.metadata?.discoveryRunId || null,
    enrichmentStartedAt:
      discoveryData.enrichmentStartedAt || discoveryData.metadata?.enrichmentStartedAt || null,
    enrichmentCompletedAt:
      discoveryData.enrichmentCompletedAt || discoveryData.metadata?.enrichmentCompletedAt || null,
    enrichmentProgressMessage:
...
const resolveDiscoveryDataForPlaylistBuild = (payload = {}) => {
  ...
  const fallback =
    payload?.discoveryData && typeof payload.discoveryData === "object"
      ...
      : {};
  const provider = cached.provider || fallback.provider || DISCOVERY_PROVIDER_LASTFM;
  return {
    provider,
    capabilities:
      cached.capabilities ||
    ...
    recommendations: pickArray(cached.recommendations, fallback.recommendations),
    ...
    fallbackGenrePools:
      cached.fallbackGenrePools && typeof cached.fallbackGenrePools === "object"
        ? cached.fallbackGenrePools
        : fallback.fallbackGenrePools && typeof fallback.fallbackGenrePools === "object"
          ? fallback.fallbackGenrePools
          : {},
    discoverPlaylists: pickArray(cached.discoverPlaylists, fallback.discoverPlaylists),
  ...
  return withHonkerLock(
    ...
    async () => {
      ...
      try {
        ...
        const { generateDiscoverPlaylists } = await import("./discoverPlaylistService.js");
        const discoverPlaylists = await generateDiscoverPlaylists({
          ...
          historyTopArtists: normalizePlaylistBuildStringList(payload?.historyTopArtists, 3),
        });
...
const scheduleDiscoverPlaylistBuild = ({
  ...
  const payload = {
    ...
    historyTopArtists: normalizePlaylistBuildStringList(playlistArgs.historyTopArtists, 3),
...
export const updateDiscoveryCache = async (options = {}) => {
  ...
  try {
    ...
    const libraryArtists =
      recentLibraryArtists.length > 0 ? recentLibraryArtists : allLibraryArtists.slice(0, 40);
    ...
    emitDiscoveryProgress("collecting_seeds", "Collecting recommendation seed artists", 20);

    const historyArtists = [];
    const defaultListenHistoryProfile = getDefaultListenHistoryProfile(dbOps.getSettings());
    ...
    const { tagsFound, topTags, topGenres, tagMap, tagWeights, genreWeights } = getLastfmApiKey()
      ? await collectSeedTagsAndGenres(profileSample, lastfmHealth, "building_genres")
      : {
          tagsFound: 0,
          topTags: [],
          topGenres: [],
    ...
    console.log(`Identified Top Genres: ${discoveryCache.topGenres.join(", ")}`);

    if (getLastfmApiKey()) {
      console.log("Fetching Global Trending (real-time style) from Last.fm...");
      console.time("[Discovery] trending fetch");
      emitDiscoveryProgress("fetching_trending", "Fetching global trending artists", 50);
      try {
          ...
          for (const track of tracks) {
            ...
            if (trackName) {
              artist.sampleTrack = {
                trackName,
                albumName:
                  String(track?.album?.title || track?.album?.["#text"] || "").trim() || null,
              };
            }
            trendingArtists.push(artist);
          }
        }
        let globalTop = mergeResolvedRecommendations(trendingArtists, existingArtistKeys).slice(
          0,
          32,
        );
        if (globalTop.length < 12) {
          ...
          if (topData?.artists?.artist) {
            ...
            globalTop = mergeResolvedRecommendations(
              [...globalTop, ...topArtists.map(buildTrendingArtistEntry).filter(Boolean)],
              existingArtistKeys,
            ).slice(0, 32);
          }
        }
        ...
        discoveryCache.globalTop = mergeResolvedRecommendations(globalTop, existingArtistKeys)
          .filter((item) => item?.id || item?.navigateTo)
          .slice(0, 32);
        console.log(`Found ${discoveryCache.globalTop.length} trending artists (from top tracks).`);
      } catch (e) {
    ...
    const recSample = seeds.slice(
      0,
      getDiscoveryRecommendationSeedLimit(seeds.length, getLastfmFailureRatio(lastfmHealth)),
    );
    const recommendationRunStartedAt = new Date().toISOString();
    const discoveryRunId = createDiscoveryRunId();

    console.log(`Generating recommendations based on ${recSample.length} seed artists...`);
    ...
    if (getLastfmApiKey()) {
      ...
      const maxResolve =
        recommendationFailureRatio >= 0.5 ? 12 : recommendationFailureRatio >= 0.3 ? 24 : 40;
    ...
    console.log(`Generated ${recommendationsArray.length} total recommendations.`);
    ...
    emitDiscoveryProgress("saving_results", "Saving discovery recommendations", 96);
    ...
    import("./aurralHistoryService.js")
      .then(({ recordDiscoveryRefreshFailed }) => recordDiscoveryRefreshFailed(error.message))
      .catch(() => {});
  } finally {
...
export const updateUserDiscoveryCache = async (listenHistoryProfile, options = {}) => {
  ...
  try {
    ...
    const _globalTop = globalCache.globalTop || [];
    ...
    recommendationsArray = rerankCachedRecommendations({
      ...
      existingRecommendations: dbOps.getDiscoveryCache(cacheNamespace).recommendations || [],
      existingArtistKeys,
      limit: getDiscoveryRecommendationPoolLimit(),
      runStartedAt: recommendationRunStartedAt,
      discoveryMode: getDiscoveryMode(),
