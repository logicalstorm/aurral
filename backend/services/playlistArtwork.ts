...
const hexToRgb = (hex: unknown): RgbColor => {
...
const mixHex = (left: unknown, right: unknown, weight = 0.5): string => {
...
const hexToRgba = (hex: unknown, alpha: unknown): string => {
...
const normalizeDisplayName = (playlistName: unknown): string =>
  String(playlistName || '')
    .replace(/^\[(?:A|AS)\]\s*/i, '')
    .replace(/^Aurral(?: Shared)?\s+/i, '')
    .trim();

const normalizeKind = (kind: unknown): string =>
  String(kind || '')
    .trim()
    .toLowerCase() === 'flow'
    ? 'Flow'
...
const chunkWord = (word: unknown, limit: number): string[] => {
  const chunks: string[] = [];
...
const isSmallTitleWord = (word: unknown): boolean => {
...
const buildTitlePhrases = (words: string[], limit: number): string[] => {
...
const buildTitleLayout = (value: unknown) => {
...
const getPalette = (playlistName: unknown) => {
...
const generateBlobPath = (cx: number, cy: number, r: number, seed: number): string => {
...
const buildArtworkSvg = ({ playlistName, kind }: { playlistName: unknown; kind: unknown }) => {
...
export async function writePlaylistArtworkWebpFromBuffer(buffer: Buffer, outputPath: string): Promise<void> {
...
export async function buildPlaylistArtworkWebpBuffer({ playlistName, kind }: { playlistName: unknown; kind: unknown }) {
...
export async function writePlaylistArtworkSidecar({ playlistName, kind, outputPath }: { playlistName: unknown; kind: unknown; outputPath: string }) {
