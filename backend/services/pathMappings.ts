...
let storedPathMappings: { source: string; remote: string; local: string }[] = [];
const PATH_MAPPING_SOURCES = new Set(['all', 'lidarr', 'slskd', 'nzbget']);

export function syncPathMappings(value: unknown) {
  storedPathMappings = normalizePathMappings(value);
}

function normalizePathSeparators(value: unknown): string {
  return String(value || '')
    .trim()
    .replace(/\\/g, '/')
    .replace(/\/+$/, '');
...
export function normalizePathMappingSource(value: unknown): string {
...
function isExplicitPathMappingSource(value: unknown): boolean {
...
function normalizePathMappingEntry(entry: Record<string, unknown>): { source: string; remote: string; local: string } | null {
...
export function normalizePathMappings(value: unknown): { source: string; remote: string; local: string }[] {
  if (!Array.isArray(value)) return [];
  const seen = new Set<string>();
  const result: { source: string; remote: string; local: string }[] = [];
...
export function getPathMappings(source: unknown = null): { source: string; remote: string; local: string }[] {
...
export function looksLikeExternalOnlyPath(value: unknown): boolean {
  const trimmed = String(value || '').trim();
  return /^[A-Za-z]:[\\/]/.test(trimmed) || /^\\\\/.test(trimmed);
}

function pathExists(targetPath: unknown): boolean {
  try {
    return fs.existsSync(targetPath as fs.PathLike);
  } catch {
    return false;
  }
}

function pathMatchesPrefix(candidate: unknown, prefix: unknown): boolean {
...
export function resolveLocalPath(externalPath: unknown, mappings = getPathMappings()): string {
...
export function resolveRemotePath(localPath: unknown, mappings = getPathMappings()): string {
...
export async function verifyMappedPath(externalPath: unknown, mappings = getPathMappings()) {
