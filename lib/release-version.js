...
export function resolveNextRelease({
  ...
  const latestStable = stableReleases[0] || parseReleaseVersion(initialStableVersion);

  if (!latestStable) {
    throw new Error(`Unable to resolve initial stable version from "${initialStableVersion}".`);
  }
